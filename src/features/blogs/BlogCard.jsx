...
function BlogCard({ article, delay, animation, ArrowOrDate }) {
  return (
    <AnimatePresence initial={animation}>
      ...
      >
        <NavLink to={`/blogs/${article?.id}`} className="flex w-full flex-col">
          ...
          <p className="mb-2 mt-6 line-clamp-2 max-h-[58px] overflow-hidden font-poppins text-xl font-medium leading-[28px] max-lg:text-start max-lg:text-sm max-sm:mt-4 max-sm:max-h-fit max-sm:min-h-fit max-sm:font-semibold">
            {article?.title}
          </p>
        </NavLink>
        {ArrowOrDate === "arrow" ? (
